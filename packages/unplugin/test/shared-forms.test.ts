...
import { matchPattern, stripDataAttributes, stripDataAttributesWithMap } from '../src/core'
...
describe('pattern metacharacters', () => {
    ...
    it('expands a wildcard over attribute-name characters only', () => {
        ...
        expect(matchPattern('data-test-', 'data-test-*')).toBe(true)
        expect(matchPattern('data-testimonial', 'data-test-*')).toBe(false)
    })

    it('matches a bare wildcard against any data attribute', () => {
        expect(matchPattern('data-anything', 'data-*')).toBe(true)
    })
})
